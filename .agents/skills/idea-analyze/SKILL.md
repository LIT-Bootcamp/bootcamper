...
The invocation is the human approval: publish `idea/vNNN.md` as the next valid immutable `human-approved IDEA version`, naming its immediate predecessor, run ID, and a non-empty reason. Validate the IDEA version before updating the manifest or changelog. Do not treat agent output as approval.
...
Validate outputs with `bin/product_factory validate --root product` before changing manifests. After all BA output is valid, publish the next immutable `analyzed IDEA version` at `idea/vNNN.md`, naming its immediate predecessor, run ID, and a non-empty reason. Validate the IDEA version before updating the manifest or changelog; then append the IDEA changelog and finish the factory run with its affected IDs. Finish unchanged input as `no-op` where allowed.
