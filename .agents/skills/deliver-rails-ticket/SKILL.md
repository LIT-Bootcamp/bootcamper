...
Factory Mode must not skip dual review, bypass remediation, expand the immutable ticket scope, or bypass `lead_bootcamper`. The lead may commit, push, and open the pull request, then must commit and push the validated `ready-for-human-merge` artifact to that same branch before the workflow stops. Only a human may merge a Factory Mode pull request; generic review skipping or agent approval is not merge authorization.
