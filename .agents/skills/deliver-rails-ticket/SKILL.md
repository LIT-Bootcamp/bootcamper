...
The ticket is complete only when acceptance criteria and required checks pass, both final verdicts are `APPROVE`, no Critical or Major finding remains, and the task record is finalized. Hand the finalized ticket to `lead_bootcamper`; only that agent may commit, push, open the pull request, or merge after human review/explicit `skip review`. Do not deploy, close an external ticket, or mutate production.
