...
The post-merge CI reconciler owns the primary immutable `done` transition and dependency recalculation. Backlog reconciliation treats Git as canonical and repairs Project drift from those versions; it must not create a second completion version for an already-done ticket. Closing or replacing a ticket with active implementation is forbidden; escalate instead. Duplicate markers, conflicting issue mappings, missing protected mappings, unauthorized targets, and partial apply failures also escalate with completed operations and exact recovery steps preserved.
