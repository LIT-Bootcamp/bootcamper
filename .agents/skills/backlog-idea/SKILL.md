...
Build operations with the pure `ProductFactory::GitHubPlan`; do not embed `gh` execution in the planner. It uses the stable `<!-- product-factory-ticket-id: TICKET-NNN -->` issue-body marker and returns a deterministic preview sorted by Ticket ID and action. Cover create, update, close superseded, reopen, unchanged no-op, managed-field repair, merged pull request completion, and escalation for ambiguous mappings or an active branch or pull request that protects implementation work.

Preview and apply are separate. Save the deterministic preview in the append-only factory journal, show all external mutations, and require explicit human authorization for the exact repository, Project, and operation set before calling GitHub. A preview-only or unchanged run performs no GitHub mutation. Before an authorized apply, verify `gh auth status` includes project scope.
...
Validate changed artifacts before manifest updates. Finish the run as `success`, `no-op`, or `escalated`, including source/output IDs, every external change, and the next action. Never change business requirements, technical scope, or merge a pull request.
